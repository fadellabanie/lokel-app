    ...
    <div class="card card-flush mt-6 mt-xl-9">
        ...
        <div class="card-header mt-5">
            <div class="card-title flex-column">
                <h3 class="fw-bolder mb-1">{{__("Interests")}}</h3>
                            ...
                            <tr wire:loading.class="opacity-50">
                                ...
                                <td>
                                    <div class="cursor-pointer symbol symbol-30px symbol-md-40px">
                                        <img src="{{asset($interest->icon)}}">
                                    </div>
                                </td>
                                <td>{{$interest->name}}</td>
                                <td>{!!isActive($interest->status)!!}</td>
                              
                                <td>{{$interest->created_at->format('m-d-Y')}}</td>
                                <td>
                                    <div class="d-flex justify-content-end flex-shrink-0">
                                        <x-edit-button href="{{route('admin.interests.edit',$interest)}}"/>
                                        <x-delete-record-button wire:click="confirm({{ $interest->id }})"/>
                                    </div>
                                </td>
                            </tr>
                            @empty
